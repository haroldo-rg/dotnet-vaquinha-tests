...
namespace Vaquinha.Tests.Common.Fixtures
{
    ...
    public class DoacaoFixture
    {
        ...
        public Doacao DoacaoValida(bool emailInvalido = false, double? valor = 5, bool maxLenghField = false, bool doadorAceitouTaxa = false)
        {            
            ...
            faker.CustomInstantiator(f => new Doacao(Guid.Empty, Guid.Empty, Guid.Empty, valor ?? (double)f.Finance.Amount(MIN_VALUE, MAX_VALUE, DECIMALS), 
                                                        PessoaValida(emailInvalido, maxLenghField), null, null, doadorAceitouTaxa));
        ...
        public Doacao DoacaoInvalida(bool doacaoAnonima = false, bool doadorAceitouTaxa = false)
        {
            var pessoaInvalida = new Pessoa(Guid.Empty, string.Empty, string.Empty, doacaoAnonima, string.Empty);
            return new Doacao(Guid.Empty, Guid.Empty, Guid.Empty, 0, pessoaInvalida, null, null, doadorAceitouTaxa);
        }
